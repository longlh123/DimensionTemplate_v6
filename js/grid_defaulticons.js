	...
	$.fn.makeTable = function($table){
		...
		$.each(objTable, function(index_row, row){
			...
			if(row['type'] == "thead")
			{
				...
				wcol_title = 40 + "%";
				wcol_cat = (60 / (numberofcolumns - 1)).toString()  + "%";
			}
			...
			$.each(row['cells'], function(index_cell, cell){
				...
				if(cell['type'] == 'thead')
				{
					if($cell.prop('class') == 'mrGridQuestionText')
					{
						$cell.removeClass('mrGridQuestionText');
						$cell.addClass('row_expand_header');
					} 
					else if($cell.prop('class') == 'mrGridCategoryText')
					{
						$cell.removeClass('mrGridCategoryText');
						$cell.addClass('column_expand_header');
					}
					else
					{
						$cell.addClass('row_expand_header');
				...
				else
				{
					if($cell.find('input').length > 0)
					{
						...
						$cell.find('input').next().next().find('input:text').removeClass('mrEdit');
						$cell.find('input').next().next().find('input:text').addClass('textbox');
					}
				}
...
$(document).ready(function(){
	...
	$('.content').children().each(function(){
		...
		if($(this).prop('class') === "Grid_DefaultIcons_namediv")
		{
			$(this).children().each(function(){
										...
										if($(this).prop('tagName').toLowerCase() === "table")
										{
											...
											$('#defaulticon_table').find('thead').children().each(function(){
												...
												$(this).children().each(function(){
													...
													if($(this).prop('class') == "row_expand_header")
													{
														row_expand_grid = true; //mrGridQuestionText
													} 
													if($(this).prop('class') == "column_expand_header")
													{
														row_expand_grid = false; //mrGridCategoryText
													} 
												});
											});
